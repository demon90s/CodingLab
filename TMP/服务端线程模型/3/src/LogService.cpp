...
void LogService::Stop()
{
	if (m_is_running)
	{
		...
		m_content_buff_cond.notify_one();
...
void LogService::WriteBuff()
{
	std::string tmp_buff;
	{
		std::unique_lock<std::mutex> lock(m_content_buff_mutex);
		if (m_content_buff.empty() && m_is_running)
		{
			m_content_buff_cond.wait(lock);
		}
		if (!m_content_buff.empty())
		{
